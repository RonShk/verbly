...
const evaluationSchemaDescriptions = TranslationPrompts.descriptions.evaluate;

const CorrectedSegmentSchema = z.object({
  text: z.string().describe(evaluationSchemaDescriptions.segment.text),
  highlight: z.enum(["none", "wrong", "correct"]).describe(evaluationSchemaDescriptions.segment.highlight),
});
...
export const evaluateTranslationResponse = functions.https.onCall(async (data, context) => {
  ...
  const questionSetRef = db.collection("translation_question_sets").doc(questionSetId);
  ...
  const prompt = TranslationPrompts.buildEvaluatePrompt(
    sentenceInLearningLanguage,
    vocabWordsUsed,
    studentAnswer
  );
